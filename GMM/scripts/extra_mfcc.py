	...
	for dr in drs:   
		...
		for sample in samples:
			...
			for filename in glob.glob(os.path.join(samples_path, '*.wav')):
                ...
                save_name = spk_dir  + '/spk_' + str(i+1) + '_mfcc.npy'
                #print(save_name)
...
# 	# female
# 	for i in range(9):
# 	    train_female_siri_file = female_siri_path + str(i+1) + '/merge_result.wav'	
# 	    save_name = '../mfcc_features/female_' + str(i+1) + '.npy'    
# 	    extraMFCC(train_female_siri_file, save_name)
# 	# male
# 	for i in range(6):
# 	    train_male_siri_file = male_siri_path + str(i+1) + '/merge_result.wav'	 
# 	    save_name = '../mfcc_features/male_' + str(i+1) + '.npy'       
# 	    extraMFCC(train_male_siri_file, save_name)
